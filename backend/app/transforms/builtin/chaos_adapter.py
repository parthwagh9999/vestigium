...
class ChaosAdapter(BaseTransform):
    id = "builtin.chaos"
    name = "ProjectDiscovery Chaos API"
    description = "Searches Chaos for subdomains (Requires API Key)"
    ...
    async def execute(self, entity: Entity, params: dict[str, Any]) -> tuple[list[Entity], list[EntityRelationship], dict[str, Any]]:
        ...
        api_key = params.get("api_keys", {}).get("CHAOS_API_KEY")
        if not api_key:
            return [], [], {"error": "CHAOS_API_KEY is required."}
        ...
        try:
            async with httpx.AsyncClient(timeout=15.0, headers=headers) as client:
                ...
                if resp.status_code == 401 or resp.status_code == 403:
                    return [], [], {"error": "Invalid Chaos API Key."}
                if resp.status_code != 200:
                    return [], [], {"error": f"Chaos API returned {resp.status_code}"}
